...
class Email(Base):
    ...
    provider_message_id = Column(String(512), nullable=False, index=True)  # Provider's message ID
    provider_thread_id = Column(String(512), nullable=True, index=True)  # Thread/conversation ID
    ...
    # Email metadata
    email_date = Column(DateTime(timezone=True), nullable=False, index=True)  # Original email date
    received_date = Column(DateTime(timezone=True), nullable=True)  # Received date
    folder = Column(String(100), nullable=True, index=True)  # Folder/label (INBOX, SENT, etc.)
    ...
    # Storage information
    raw_mime_storage_path = Column(String(512), nullable=True)  # S3/MinIO path for raw MIME
    ...
    importance_score = Column(Float, nullable=True)  # Importance score (0.0 - 1.0)
    importance_level = Column(
        Enum(ImportanceLevel), 
        nullable=True, 
        index=True
    )
    classification = Column(JSON, nullable=True)  # Classification results (category, tags, etc.)
    sentiment = Column(String(50), nullable=True)  # Sentiment analysis (positive, negative, neutral)
    
    # Vector DB
    vector_db_id = Column(String(255), nullable=True, index=True)  # Vector DB document ID
    ...
    auto_action = Column(String(50), nullable=True)  # Auto action taken (delete, archive, tag, etc.)
    ...
    fetched_at = Column(DateTime(timezone=True), server_default=func.now(), nullable=False)  # When email was fetched
    ...
    __table_args__ = (
        ...
        Index('idx_email_account_unprocessed', 'email_account_id', 'is_processed'),
        Index('idx_email_date_status', 'email_date', 'status'),
        Index('idx_provider_message', 'email_account_id', 'provider_message_id', unique=True),
    )
