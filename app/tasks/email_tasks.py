...
def fetch_emails_task(account_id: int, provider_type: str, credentials: dict, since: Optional[datetime] = None):
    ...
    try:
        ...
        try:
            ...
            db.rollback()
            logger.error(f"Error saving emails to DB for account {account_id}: {e}")
            raise e
        finally:
            db.close()
...
def fetch_emails_for_account(account_id: int):
    ...
    try:
        ...
        if result.get("status") == "success":
            ...
            account_repo.update_last_fetch(account_id, success=False, error_message=error_msg)
            logger.error(
        ...
        logger.exception(f"Error in fetch_emails_for_account for account {account_id}: {e}")
        try:
            account_repo = EmailAccountRepository(db)
            account_repo.update_last_fetch(account_id, success=False, error_message=str(e))
        except:
            pass
...
def fetch_all_accounts_emails():
    ...
    try:
        ...
        for account in active_accounts:
            ...
            if account.last_fetch_at:
                time_since_last_fetch = (datetime.now() - account.last_fetch_at).total_seconds()
            ...
            logger.info(f"Triggered email fetch for account {account.id} ({account.email_address})")
