...
def vector_search_node(state: EmailProcessingState) -> EmailProcessingState:
    ...
    try:
        ...
        if not text_for_embedding:
            logger.warning(f"No text available for embedding email {state['email_id']}")
            state["vector_db_id"] = None
            state["embedding_model"] = None
            state["similar_emails"] = None
            state["vector_metadata"] = None
        ...
        logger.info(f"Vector stored for email {state['email_id']} with ID: {doc_id}")
        ...
        logger.error(f"Error in vector search for email {state['email_id']}: {e}", exc_info=True)
...
def _prepare_text_for_embedding(state: EmailProcessingState) -> str:
    ...
    body = state.get("processed_body_html") or state["email_data"].get("body_text", "")
    if body:
        # 너무 긴 본문은 잘라서 처리
    ...
    try:
        ...
        filter_condition = Filter(
            must=[
                ...
                FieldCondition(key="email_id", match=MatchValue(value=str(current_email_id)), match_not=True)
            ]
        )
        
        # 유사도 검색 (LangChain 1.0: filter는 query_filter로 전달)
        # LangChain 1.0에서는 Qdrant 필터를 query_filter 파라미터로 전달
        results = vector_store.similarity_search_with_score(
            query=query_text,
            k=limit,
            query_filter=filter_condition  # LangChain 1.0: filter -> query_filter
        )
